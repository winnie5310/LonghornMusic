...
namespace LonghornMusic.Controllers
{
    public class ArtistsController : Controller
    {
        ...
        {
            SongIndexData viewModel = new SongIndexData();

            viewModel.Artists = db.Artist
                .Include(a => a.Genres)
                .Include(a => a.Songs)
                .OrderBy(a => a.Name);

            //add instructor data when user specifies an id
            if (id != null)
            {
                ViewBag.ArtistID = id.Value;
                viewModel.Genres = viewModel.Artists.Where(a => a.ArtistID == id).Single().Genres;
                viewModel.Songs = viewModel.Artists.Where(x => x.ArtistID == id).Single().Songs;
            }
            else 
            {
                ViewBag.ArtistID = null;
            }
